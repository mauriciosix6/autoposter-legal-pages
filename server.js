...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    h1 { color: var(--cyan); margin-top: 0; font-size: 34px; }
    h2 { color: var(--cyan); margin-top: 28px; font-size: 21px; }
    p, li { color: var(--text); font-size: 16px; }
    .muted { color: var(--muted); }
    a { color: var(--pink); text-decoration: none; }
    a:hover { text-decoration: underline; }
    .nav { margin-top: 28px; padding-top: 18px; border-top: 1px solid rgba(255,255,255,0.1); }
...
app.get("/terms", (req, res) => {
  res.type("html").send(page(`Termos de Serviço - ${APP_NAME}`, `
    ...
    <p>Quando conectado ao TikTok, o ${APP_NAME} pode solicitar autorização oficial para enviar vídeos usando permissões concedidas pelo usuário, como upload de vídeo para publicação ou confirmação posterior dentro do aplicativo TikTok.</p>

    <h2>5. Publicação e aprovação</h2>
    <p>Em determinados fluxos, como envio para TikTok Inbox, o usuário pode precisar finalizar, revisar ou aprovar a publicação dentro do aplicativo TikTok.</p>
    ...
    <h2>7. Suspensão ou encerramento</h2>
...
app.get("/privacy", (req, res) => {
  res.type("html").send(page(`Política de Privacidade - ${APP_NAME}`, `
    ...
    <p>Quando o usuário conecta uma conta TikTok, o ${APP_NAME} utiliza tokens oficiais de autorização somente para executar ações permitidas pelo usuário, como upload de vídeo. O usuário pode revogar o acesso pelas configurações da própria plataforma TikTok.</p>
    ...
    <p>Algumas informações podem ser salvas localmente no computador do usuário, como sessões, tokens, arquivos de configuração, histórico de status e preferências de uso.</p>
    ...
    <p>O usuário pode remover tokens locais, desconectar contas ou revogar permissões diretamente nas plataformas conectadas.</p>
